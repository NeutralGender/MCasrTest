    ...
    for(std::size_t i = 0; i < loop; ++i)
    {
        for(auto k = 0; k < v.size(); k+=package)
        {
            ...
            for(auto& ch : channels)
            {
                ...
                for(auto p = k; p < (k + package) && (p != v.size()); p+=package_per_tact)
                {
                    for( auto pt = p; pt < ( p + package_per_tact ) && (pt != v.size()); ++pt )
                    {
                        ++atom;
                        ++_consume_ix_;
                        ch->send(v[p].c_str());
                    }
                    for( std::size_t i = 0; i < tact_us; ++i )
                    { }
                }
            }
            std::this_thread::sleep_for(std::chrono::microseconds(delay));
    ...
    for( std::size_t i = 0; i < loop; ++i )
    {
        for(auto k = 0; k < v.size(); k+=package)
        {
            ...
            for(auto& ch : channels)
            {
                ...
                for(auto p = k; p < (k + package) && (p < v.size()); p+=package_per_tact)
                {
                    for( auto pt = p; pt < ( p + package_per_tact ) && (pt < k+package) && (pt < v.size()); ++pt )
                    {
                        ++atom;
                        ++_consume_ix_;
                        ch->send(v[pt].c_str());
