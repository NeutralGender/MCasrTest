...
void timer(const std::size_t delay_us, const char* str_delay)
{
    auto start_time = std::chrono::system_clock::now();
    for( std::size_t i = 0; i < delay_us; ++i )
    {
            
    }
    auto end_time = std::chrono::system_clock::now();
    std::chrono::duration<double, std::micro> fp_ms = end_time - start_time;
    std::cout << str_delay << fp_ms.count() << std::endl;
}
...
int main()
{
    ...
    if( mconf.delay_us != 0 )
    {
        timer(mconf.delay_us, "Delay test: ");
        poll();

        timer(mconf.tact_us, "Tact test: ");
        poll();
